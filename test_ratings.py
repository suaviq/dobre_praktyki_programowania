...
class TestRatingsEndpoints:
    ...
    def test_get_ratings_returns_all_items(self, client, sample_ratings):
    ...
    def test_get_ratings_empty_database(self, client):
    ...
    def test_get_rating_by_id_success(self, client, single_rating):
        """Test GET /ratings/{rating_id} - zwraca ocenę o podanym ID"""
        response = client.get(f"/ratings/{single_rating.id}")
    ...
    def test_create_rating_success(self, client, db_session):
        ...
        verify_response = client.get(f"/ratings/{data['id']}")
        assert verify_response.status_code == 200

    def test_create_rating_without_timestamp(self, client):
    ...
    def test_update_rating_success(self, client, single_rating):
        """Test PUT /ratings/{rating_id} - aktualizuje ocenę"""
        update_data = {
            "rating": 5.0
        }
        
        response = client.put(f"/ratings/{single_rating.id}", json=update_data)
        
        assert response.status_code == 200
        data = response.json()
        assert data["rating"] == 5.0
        
        verify_response = client.get(f"/ratings/{single_rating.id}")
    ...
    def test_delete_rating_success(self, client, single_rating):
        """Test DELETE /ratings/{rating_id} - usuwa ocenę"""
        response = client.delete(f"/ratings/{single_rating.id}")
