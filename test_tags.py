...
class TestTagsEndpoints:
    ...
    def test_get_tags_returns_all_items(self, client, sample_tags, auth_headers):
    ...
    def test_get_tags_empty_database(self, client, auth_headers):
    ...
    def test_get_tag_by_id_success(self, client, single_tag, auth_headers):
        """Test GET /tags/{tag_id} - zwraca tag o podanym ID"""
        response = client.get(f"/tags/{single_tag.id}", headers=auth_headers)
    ...
    def test_create_tag_success(self, client, db_session, auth_headers):
        ...
        verify_response = client.get(f"/tags/{data['id']}", headers=auth_headers)
        assert verify_response.status_code == 200

    def test_create_tag_without_optional_fields(self, client, auth_headers):
    ...
    def test_update_tag_success(self, client, single_tag, auth_headers):
        """Test PUT /tags/{tag_id} - aktualizuje tag"""
        update_data = {
            "tag": "updated tag"
        }
        
        response = client.put(f"/tags/{single_tag.id}", json=update_data, headers=auth_headers)
        
        assert response.status_code == 200
        data = response.json()
        assert data["tag"] == "updated tag"
        
        verify_response = client.get(f"/tags/{single_tag.id}", headers=auth_headers)
    ...
    def test_update_tag_not_found(self, client, auth_headers):
        ...
        response = client.put("/tags/99999", json=update_data, headers=auth_headers)
        
        assert response.status_code == 404

    # ==================== DELETE ====================

    def test_delete_tag_success(self, client, single_tag, auth_headers):
        """Test DELETE /tags/{tag_id} - usuwa tag"""
        response = client.delete(f"/tags/{single_tag.id}", headers=auth_headers)
